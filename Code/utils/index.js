...
export function fullScreen() {
  let el = document.getElementById('sample_content').getElementsByClassName('el-card__body')[0];
  // 全屏后黑屏了，设置el-card__body当前元素的背景颜色为白色
  ...
  if (window.ActiveXObject) {
    ...
    el.webkitRequestFullScreen();// Webkit (works in Safari5.1 and Chrome 15)
    // 写全屏后的执行函数
...
export function generateLineageTitle(command, name) {
  let map = {
    forward: 'Usage',
    backward: 'Reference',
    full: 'Ref&Usage'
  }
  return `${map[command]}-${name}`
}

/* 生成图片 */
export function generateImages(width, height, myDiagram) {
  // sanitize input
  ...
  let db = myDiagram.documentBounds.copy();
  let boundswidth = db.width;
  let boundsheight = db.height;
  let imgWidth = width;
  let imgHeight = height;
  let p = db.position.copy();
  let img;
