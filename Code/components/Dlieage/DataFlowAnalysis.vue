    ...
    methods: {
      ...
      queryDiagramData() {
        ...
        get(url, {
          ...
          parseAjaxResponse(res, () => {
            ...
            // 去掉收尾是同一结点的线
            let linkData = res.data.linkDataArray.filter(item => item.fromPort !== item.toPort)
            let existNewNode = _this.appendNewNodes(nodeData, linkData)
            if (!existNewNode) {
              _this.$message.success('没有更多数据了')
              return
            }
            _this.layout()
          })
      ...
      load() {
        ...
        if (this.showEtl === '0') {
          ...
          // 隐藏etl
          let etlArr = []
          this.diagramData.nodeDataArray = this.nodeDataArray.filter(item => {
            if (item.category === 'EtlNode') {
              etlArr.push(item.key)
              return false
            } else {
              return true
            }
          ...
          etlArr.forEach(etlId => {
            linkNode = []
            _this.linkDataArray.forEach(item => {
              if (item.from === etlId || item.to === etlId) {
                linkNode.push(item)
              }
            })
            if (linkNode.length === 2) {
          ...
          // 过滤掉收尾是同一个结点的线
          this.diagramData.linkDataArray = newLinkDataArray.filter(item => item.fromPort !== item.toPort)
        }
