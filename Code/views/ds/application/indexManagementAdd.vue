...
export default {
  ...
  methods: {
    ...
    getIndexBasicAttrInfos() {
      ...
      get("/ds/application/index/getBasicAttrInfos").then(res => {
        parseAjaxResponse(res, () => {
          if (res.code === 10000) {
            // let indexTech = 1;
            ...
            this.inputData.map((val, index) => {
              ...
              //   // console.log(e, val.columnName)
              //   this.baseInfoRuleForm[val.columnName] = e;
              //   if (val.columnName === this.oBizTerm.sCnName) {
              //     // 中文名称
              //     get("/ds/bizTerm/queryHomologousEnVocabularies", {
              //       cnTerm: e
              //     }).then(res => {
              //       if (res.code === 10000) {
              //         // console.log(this.eIndex, 'this.eIndex')
              //         this.$set(
              //           this.inputData[this.eIndex],
              //           "attrValue",
              //           res.data[0]
              //         );
              //       }
              ...
              if (val.columnName === 'NAMESPACE') {
                ...
                this.$set(this.inputData[index], 'nodeList', this.route.nodeList)
              }
            ...
            console.log(this.inputData2, this.baseInfoRules, 'this.inputData')
            bus.$on('columnName', (e) => {
              let key;
              let key2;
            ...
            if (
              this.route.msg.namespace &&
              this.route.msg.namespaceCN
            ) {
              this.inputData.map((val, index) => {
                if (val.columnName === "NAMESPACE") {
                  this.$set(
                ...
                if (val.columnName === "INDEX_TYPE") {
                  // 指标类型 id - InfoItem 默认值
                  this.sIndexType = this.route.msg.indexInfoItem

                  this.$set(
                    this.inputData[index],
                    "attrValuName",
                    this.route.msg.select.name
                  );
                  this.$set(
                    this.inputData[index],
                    "select",
                    this.route.msg.select
                  );
                }
              });
            }
            console.log(this.inputData, 'this.inputData2')
          }
        });
      });
    },
    ...
    sendSaveMsg() {
      ...
      postByJson("/ds/application/index/saveBasicInfos", params).then(
        res => {
          parseAjaxResponse(res, () => {
            if (res.code === 10000) {
    ...
    getParMsg(val) {
      ...
      if (msg) {
        ...
        this.firstNodeList = msg.nodeList ? msg.nodeList : null
        if (msg.data) {
      ...
      bus.$on('getNodeArr', (msg) => {
        console.log(msg === this.route.nodeList, 'ddddd')
        if (msg === this.firstNodeList) {
          this.addNode = false
        } else {
          this.addNode = true
        }
