<!--指标管理-->
<template>
  <div @click.capture="clickBox" class="index-management dashboard-editor-container tableNoPadding">
    <el-container>
      <el-aside width="20%" class="leftdiv">
        <!--右键tree节点，显示的菜单box-->
        <div ref="rightMenu" v-show="showMenu" class="context-menu">
          <el-card>
            <el-row v-show="bEditAndDeleteIsShow" @click.native="getCatalogDetail(false)">详细信息</el-row>
            <el-row v-permission="'INDEX_ADD_CATALOG'" @click.native="toAddCatalog">新建目录</el-row>
            <el-row
              v-permission="'INDEX_EDIT_CATALOG'"
              v-show="bEditAndDeleteIsShow"
              @click.native="toEditCatalog"
            >修改目录</el-row>
            <el-row
              v-permission="'INDEX_DEL_CATALOG'"
              v-show="bEditAndDeleteIsShow"
              @click.native="showDeleteDialog"
            >删除目录</el-row>
            <el-row v-permission="'INDEX_ADVANCED_SEARCH'" v-show="bEditAndDeleteIsShow">高级搜索</el-row>
            <el-row v-permission="'INDEX_IMPORT'" v-show="bEditAndDeleteIsShow">导入标准</el-row>
            <el-row v-permission="'INDEX_EXPORT'"  @click.native="exportData">导出标准</el-row>
          </el-card>
        </div>
        <el-card>
          <div class="header">
      ...
      <el-main style="margin-left:15px;">
        ...
        <el-card class="card-container" v-show="showComponent==='card' || showComponent==='cardMain'">
          ...
          <div class="button-box">
            <!--            <el-button type="primary" size="mini" class="btn" @click="toDetails"><svg-icon icon-class="yulan" ></svg-icon> <span class="btn-text">详情</span> </el-button>-->
            <el-button v-permission="'INDEX_ADD'" type="primary" size="mini" class="btn add" @click="toAdd">
            ...
            <!--            <el-button type="primary" size="mini" class="btn" @click="toExport"><svg-icon icon-class="xiazai" ></svg-icon> <span class="btn-text">导出</span> </el-button>-->
            <!--            <el-button type="primary" size="mini" class="btn" @click="toImport"><svg-icon icon-class="shangchuan" ></svg-icon> <span class="btn-text">导入</span> </el-button>-->

            <!-- 血缘分析 按钮 -->
            <el-dropdown @command="handleBlood">
              <el-button
                class="basic-button"
                style="width: 75px;padding: 0px 10px 0 18px;height: 30px;"
                type="primary"
              >
            ...
            <!--<el-form style="height: 30px; width:240px;float:right;" label-width="55px">
              <el-form-item label="关键字：">
                <el-input clearable v-model="keyWord" placeholder="请输入内容..." size="mini"
                          @keyup.enter.native="toSearch"
                >
                  <i slot="suffix" style="cursor: pointer" class="el-input__icon el-icon-search" @click="toSearch"></i>
                </el-input>
              </el-form-item>
            </el-form>-->
          </div>
          <div class="card-table" v-loading="oIndexData.bLoading">
            <el-table
              v-if="!oIndexData.bLoading"
              border
              size="mini"
              align="center"
              :data="oIndexData.aTableData"
              style="width: 100%"
              ...
              <el-table-column label="操作" align="center" fixed="right" width="160">
                <template slot-scope="scope">
                  ...
                  <el-tooltip class="item" effect="dark" content="编辑" placement="bottom">
                    <el-button v-permission="'INDEX_EDIT'" type="text" @click="toEdit(scope.row)">
                  ...
                  <el-tooltip class="item" effect="dark" content="删除" placement="bottom">
                    <el-button v-permission="'INDEX_DEL'" @click='deleteSome(scope.row)' type="text" size="small" >
        ...
        ></catalog-detail>
        <index-management-edit v-if="showComponent==='editorDetail'" ref="detailModsule" @colse="showComponent='card'"></index-management-edit>
      </el-main>
    </el-container>
    ...
    >
      <div v-show="TitleTitle" slot="title">新建目录</div>
      <div v-show="!TitleTitle" slot="title">修改目录</div>
      <el-form
    ...
    <!-- 操作弹框 -->
    <el-dialog :visible.sync="addData.addDialog" class="new-editor-container" :title="dialogTitle" width="50%" >
      <div class="body">
        <index-management-add ref="addModsule" v-if="addData.addDialog&&dialogTitle==='新增'"></index-management-add>
        <index-management-basic-editor ref="editModsule" v-else-if="addData.addDialog&&dialogTitle==='编辑'"></index-Management-basic-editor>
      </div>
      <div class="dialog-btn">
...
import indexManagementAdd from "./indexManagementAdd";
import indexManagementBasicEditor from "./indexManagementBasicEditor";
import indexManagementEdit from "./indexManagementEdit";
...
export default {
  ...
  data() {
    ...
    return {
      ...
      aIndexTypeList: [{ id: 1, name: "test1" }, { id: 2, name: "test2" }],
      aBusinessList: [{ id: 1, name: "test1" }, { id: 2, name: "test2" }],
      aDepartmentList: [{ id: 1, name: "test1" }, { id: 2, name: "test2" }],
      aIndexStateList: [{ id: 1, name: "test1" }, { id: 2, name: "test2" }],
      oIndexData: {
        aTableData: [],
        bLoading: false
      },
      ...
      // 四种指标对应的InfoItem
      constIndex: {
        BaseIndex: 'BaseIndexInfoItem',
        DeriveIndex: 'DeriveIndexInfoItem',
        RuleIndex: 'RuleIndexInfoItem',
        CalculateIndex: 'CalculateIndexInfoItem'
      },
      exportId: '',
      fileName: '',
      viewId: '',
      nodeForFileName: '',
      dialogTitle: null, // 判断编辑弹框是新增还是修改
      addData: {
  ...
  methods: {
    ...
    exportData() {
      ...
      downloadFile('/ds/application/index/downIndex', {
              ...
              res.data.map(item => {
                if (item.classifierId === this.classifyNameId) {
                  ...
                  this.editInputList.map(i => {
                    ...
                    if (this.TitleTitle && i.attCode === 'Is_Entity' || this.TitleTitle && i.attrCode === 'Is_Entity') {
                      this.$set(
                        this.baseInfoRuleForm,
                        i.attrStore,
                        'F'
    ...
    refreshLazyTree(node) {
      ...
      if ((!this.viewId === '0' && node.data === undefined) || node.data.notetype === 'view') { // 调用外部树接口
        url = '/ds/application/index/dsIndexTreeOut'
      } else { // 调用内部树接口
        url = '/ds/application/index/dsIndexTreeInner'
      }
    ...
    // Tree懒加载 - 获取层级关系树结构外部树 /ds/application/index/dsIndexTreeOut
    loadNode(node, resolve) {
      this.node = node;
      if (node.level === 0) {
        this.nodeFirst = node;
        this.resolve = resolve;
        get("/ds/application/index/dsIndexTreeOut", {
          // 初始化页面的tree时，id和nodeType都为空
          id: this.sViewId,
          nodeType: this.sNodeType
        }).then(res => {
          if (res.code === 10000 && res.success === true) {
            if (res.data.length > 1 && res.data[1].classifierId) {
              this.namespace = res.data[1].namespace
              this.toAddMsg.namespace = res.data[1].namespace;
              this.toAddMsg.node = res.data[1];
              this.toAddMsg.indexInfoItem = this.constIndex.BaseIndex // 新增-不选择目录时， 默认'基础指标'
              this.toAddMsg.select = res.data[1]
            }
            this.treeLoading = false;
            this.bIsButtonDisabled = false; // 刷新按钮 不失效
        ...
        if (node.data.notetype === "folder" || node.data.notetype === "view") {
          // 外部树
          get("/ds/application/index/dsIndexTreeOut", params).then(res => {
            if (res.code === 10000 && res.success === true) {
              this.treeLoading = false;
              this.bIsButtonDisabled = false; // 刷新按钮 不失效
              return resolve(res.data);
          ...
          node.data.notetype === "class"
        ) {
          // 内部树
          // 获取主数据层级关系树结构内部树 /ds/application/index/dsIndexTreeInner
          get("/ds/application/index/dsIndexTreeInner", params).then(res => {
            if (res.code === 10000 && res.success === true) {
              this.treeLoading = false;
              this.bIsButtonDisabled = false; // 刷新按钮 不失效
              return resolve(res.data);
    ...
    // 点击Tree节点
    handleNodeClick(data, node, compoment) {
      ...
      if (data.classifierId) {
        if (data.classifierId.includes("Parent")) {
          console.log(data);
          this.toAddMsg.select = data;
          // 四种指标的 InfoItem 判断
          if (data.name.includes("基础指标")) {
            this.toAddMsg.indexInfoItem = this.constIndex.BaseIndex
          } else if (data.name.includes("规则指标")) {
            this.toAddMsg.indexInfoItem = this.constIndex.RuleIndex
          } else if (data.name.includes("衍生指标")) {
            this.toAddMsg.indexInfoItem = this.constIndex.DeriveIndex
          } else if (data.name.includes("计算指标")) {
            this.toAddMsg.indexInfoItem = this.constIndex.CalculateIndex
          }
    ...
    handleContextmenu(event, data, node, component) {
      ...
      this.nodeForFileName = node
      if (this.viewId === '0') { // 全局视图四种指标分类的level为2
        if (this.nodeForFileName.level > 2) {
          while (this.nodeForFileName.level > 2) {
            this.nodeForFileName = this.nodeForFileName.parent
          }
        }
      } else {
        if (this.nodeForFileName.level > 1) {
          while (this.nodeForFileName.level > 1) {
            this.nodeForFileName = this.nodeForFileName.parent
          }
        }
      }
      this.fileName = this.nodeForFileName.data.name;
    ...
    getInfoItem() {
      ...
      this.oIndexData.bLoading = true;
      if (this.classifyNameId === "") {
        this.classifyNameId = "Parent_Index";
      }
      let params = {
      ...
      console.log(params.classifierId, "this.classifyNameId");
      this.getSearchList();
      get("/ds/customSearch", params).then(res => {
        if (res.code === 10000) {
          if (count === this.count) {
            ...
            this.oIndexData.aTableData = res.data.records;
            this.oIndexData.bLoading = false;
    ...
    // 删除指标标准 /ds/application/index/removeIndex
    getRemoveIndex(ids) {
      const load = loading("删除中...");
    ...
    toEdit(row) {
      ...
      this.$nextTick(() => {
        this.$refs.editModsule.getParMsg(params);
      })
      // this.$router.push({
      //   name: "indexManagementEdit",
      //   params: {
    ...
    handleSure(type) {
      console.log(type)
      if (type === '新增') {
        this.$refs.addModsule.saveBasicInfo();
      } else if (type === '编辑') {
        console.log(this.$refs.editModsule)
        this.$refs.editModsule.saveBasicInfo();
      }
    },
    // activated操作
    setTreePosition(val) {
      ...
      if (val.nodeList && val.nodeList.length > 0 && val.addNode) {
      // 再次新增规则   且  挂载点（路径）改变了
      ...
      } else if (val.nodeList && val.nodeList.length > 0 && !val.addNode) {
      // 再次新增规则   且  挂载没变
