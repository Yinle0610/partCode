          ...
          <div class="left-box">
            <el-card shadow="hover">
              ...
              <div class="md-filter">
                ...
                >
                  <i slot="prefix" class="el-input__icon el-icon-search" @click="goQueryPage"></i>
                </el-input>
              </div>
              <div class="md-tree">
                <el-tree
              ...
              <div class="view-filter">
                ...
                <el-select v-model="sViewId" placeholder="请选择" size="small" @change="changeView">
                  <el-option
        ...
        <el-main>
          ...
          <div v-show="showComponent === 'main'" class="right-box">
            <el-card shadow="hover">
              ...
              <svg-icon v-if="sNodeType !== 'metadata'"  icon-class="nodata" class="nodata"></svg-icon>
              <div v-if="sNodeType === 'metadata' && !bShowEdit&&!bshowDeltail">
                ...
                <div class="card-button">
                  <el-button @click="goEdit('main')" type="warning" size="mini" class="xiugai" v-permission="'MD_EDIT'">
                  ...
                  <el-dropdown @command="handleBlood" v-if="bShowBtn" v-permission="'MD_ANALYSIS'">
                    ...
                    <el-dropdown-menu slot="dropdown" trigger="click" class="metadataAnalyze md_browse">
                      <el-dropdown-item v-if="showDataLineage" :command="'dataLineage'">数据血缘</el-dropdown-item>
                      <el-dropdown-item :command="'anaalysis'">影响分析</el-dropdown-item>
                      <el-dropdown-item :command="'blood'">血统分析</el-dropdown-item>
                    </el-dropdown-menu>
                  </el-dropdown>
                  <el-button @click="showComponent = 'subEditor'" v-if="showExportBtn" v-permission="'MD_SUB'" type="primary" size="mini" class="xiugai">
                    <svg-icon icon-class="sub"></svg-icon>
                    <span class="text" style="bottom: 4px;">订阅</span>
                  </el-button>
                  <el-button v-permission="'MD_VERSION'" @click="handleVersion" v-if="showVersionBtn" type="primary" size="mini" class="xiugai">
              ...
              <div v-if="bshowDeltail" class="deltail-box">
                ...
                <div class="deltail-body clearfix">
                  <div class="card-button">
                    ...
                    <el-dropdown @command="handleBlood2"
          ...
          <div v-show="showComponent === 'metaEdit'" class="meta-edit">
            <meta-edit @changeShow='backChangeShow' @changeComponentShow='changeComponentShow' :formValue="formValue"></meta-edit>
          </div>
          <div class="version-add" v-show="showComponent === 'versionAdd'">
            <div class="version-header">
              <el-button @click="showComponent = 'main'" class="edit-back">返回</el-button>
            </div>
            <version-add v-loading="versionLoading" :tableData='versionTableData' :list='versionList'></version-add>
          </div>
          <div v-if="showComponent === 'subEditor'" class="subEditor">
            <el-card>
              <div class="header">
                <el-button size="mini" @click="backDetail">返 回</el-button>
                <el-button  type="primary" size="mini" style="position: absolute;right: 8px;" @click="saveSub">保 存</el-button>
              </div>
              <div class="body">
                <new-subscription-model ref="subscription" @close="backDetail"></new-subscription-model>
              </div>
            </el-card>
          </div>
        </el-main>
...
import newSubscriptionModel from "./newSubscriptionModel";
import versionAdd from "./versionAddCom"
...
export default {
  ...
  methods: {
    ...
    handleBlood(command) {
      if (command === "anaalysis") {
        if (this.sSvgItemId === null) {
          // 显示 EDGS 的 血缘分析图
          this.goInfluence(this.sCurrentId, this.sCode1);
        } else {
          // 显示 svg 的 血缘分析图
          this.goInfluence(this.sSvgItemId, this.sCode1);
        }
      } else if (command === "blood") {
        if (this.sSvgItemId === null) {
          // 显示 EDGS 的 血缘分析图
          this.goAncestry(this.sCurrentId, this.sCode1);
        } else {
          // 显示 svg 的 血缘分析图
          this.goAncestry(this.sSvgItemId, this.sCode1)
        }
      } else if (command === 'dataLineage') {
        this.$router.push({
    ...
    // 订阅保存
    saveSub() {
      this.$refs.subscription.onBackSubmit('form');
    },
      ...
      get("/md/listOwnViewsForSelect").then(res => {
        if (res.code === 10000 && res.success === true) {
          ...
          res.data.map((val, index) => {
            if (val.defaultShow) {
              this.sViewId = val.id;
            } else if (index === res.data.length - 1 && this.sViewId === null) {
              this.sViewId = '0'
            }
          })
        } else {
    ...
    handleNodeClick(data, node, resolve) {
      ...
      if (data.nodeType === "metadata") {
        ...
        this.getSVGItemId(this.sCurrentId)
  ...
  computed: {
    ...mapGetters(["roles"]),
    showDataLineage () {
      let canShowDataLineageClassifierId = [
        'Table', 'View', 'Column'
      ];
      return canShowDataLineageClassifierId.includes(this.sCurrentClassId);
    }
  },
  mounted() {},
  created() {
