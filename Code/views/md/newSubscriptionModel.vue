                ...
                <div class="button-box">
                    ...
                    <el-form class="bt-form">
                      <el-table :data="tableData.data"  style="width: 100%;margin-top:20px;" :header-cell-style="{background:'#f8f8f9'}"
                        ...
                        <el-table-column prop="code" label="分支代码"  width="170" show-overflow-tooltip>
                        </el-table-column>
                        <el-table-column prop="name" label="分支名称"  width="180" show-overflow-tooltip>
                        </el-table-column>
                        <el-table-column prop="classifierId" label="分支所属元数据类型"  width="150" align="center">
                        </el-table-column>
                        <el-table-column prop="path" label="分支上下文路径"  show-overflow-tooltip>
                        </el-table-column>
                        <el-table-column label="订阅元数据类型"  align="center">
                          <template slot-scope="scope">
                            <el-form-item style="margin-bottom:0;">
                              <el-select v-model="type[scope.$index]" placeholder="全部" size="small" @change="getType(scope.$index)" multiple style="min-width:142px;">
                ...
                <div class="button-box">
                    ...
                    <el-form class="bt-form">
                      <el-table :data="peopleTableData.data"  style="width: 100%;margin-top:20px;" :header-cell-style="{background:'#f8f8f9'}"
                        ...
                        <el-table-column prop="title" label="用户名称"  show-overflow-tooltip>
                        </el-table-column>
                        <el-table-column prop="parent" label="所属部门" show-overflow-tooltip>
                          <template slot-scope="scope">
...
import subDataList from './../portal/subscribe/subDataList.vue'
import pinyin from 'pinyin'
export default {
  name: 'newSubscriptionModel',
  ...
  methods: {
        ...
        if (valid) {
          if (!this.locking) {
            ...
            for (let i = 0; i < this.tableData.data.length; i++) {
              let obj = {
                instanceId: this.tableData.data[i].id,
            ...
            console.log(params)
            // debugger
            postByJson('/home/subscribe/createMySubscribe', params).then(res => {
    ...
    saveSubOne() {
      if (this.checkData.data !== null) {
        this.tableData.data.push(this.checkData.data);
        this.type.push(null);
        if (!this.form.name) {
          this.form.name = '订阅_' + this.checkData.data.name
        } else {
          this.form.name += '_' + this.checkData.data.name
        }
        if (!this.form.description) {
          this.form.description = '订阅_' + this.checkData.data.name
        } else {
          this.form.description += '_' + this.checkData.data.name
        }
    ...
    getNewClassifer() {
      ...
      this.getClassifierData(this.checkData.selectClassiferId)
    },
    handleNodeClick(data, node, item) {
      console.log(data, node)
      if (data.classifierId && data.classifierId === "Schema") {
        this.ifDisabled = false;
        this.checkData.selectClassiferId = data.id;
        this.toSubscribeList.iCurrentPage = 1;
        this.getSelectId(data);
        this.keyword = null;
        this.classifierId = null;
        this.getClassifier(data.id)
        this.getClassifierData(data.id)
      }
    },
    ...
    // 获取当前用户信息
    getCurrentUser() {
      get('/home/subscribe/getCurrentSubscriberInfos').then(res => {
        parseAjaxResponse(res, () => {
          this.peopleTableData.currentUser = res.data;
          let obj = {
            title: res.data.username,
            userId: res.data.userId,
            parent: res.data.depName,
            parentId: res.data.depId,
            email: res.data.email
          }
          this.peopleTableData.data.push(obj)
          this.wData.filterSponsorArr.push(obj)
          this.wData.pfilterSponsorArr.push(res.data.userId)
        });
      })
    },
    isActive(route) {
