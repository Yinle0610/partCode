...
export default {
  ...
  methods: {
    ...
    showTreeData(val) {
      ...
      if (val) {
        get(this.inputItem.columnTypeValue, {
          indexType: this.$parent.$parent.$parent.$parent.sIndexType
        }).then(res => {
          if (res.code === 10000 && res.success) {
            this.aTreeData = res.data
          } else {
    ...
    if (this.inputItem.columnType === 'combobox') {
      get('/ds/listStandardAttrComboboxInfos', {
        ...
        if (res.code === 10000 && res.success) {
          ...
          } else if (this.inputItem.columnName === 'STRING_4') {
            // 是否必填
            console.log(this.comboboxData)
            if (this.inputItem.attrValue) {
              this.inputData = this.inputItem.attrValue
            } else {
              this.comboboxData.map(item => {
                if (item.textField === '否') {
          ...
          } else if (this.inputItem.columnName === 'STRING_2') {
            // 发布状态
            if (this.inputItem.attrValue) {
              this.inputData = this.inputItem.attrValue
            } else {
              this.comboboxData.map(item => {
                if (item.textField === '待发布') {
                  this.inputData = item.valueField
                }
              })
            }
          } else if (this.inputItem.columnName === 'STRING_7') {
            // 关键属性
            if (this.inputItem.attrValue) {
              this.inputData = this.inputItem.attrValue
            } else {
