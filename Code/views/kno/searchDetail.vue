  ...
  <div class="hole">
    <el-container class="search-box">
      <el-aside width="240px" class="rightPart">
        ...
        <!-- <el-card class="box-card titleBox">
          <div slot="header" class="clearfix">
            <span>标签信息</span>
          </div>
          <el-input
            @keyup.enter.native="handleSearchTitle"
            placeholder="请输入内容"
            v-model="searchTitle"
            class="input-with-select"
          >
            <i @click="handleSearchTitle" slot="suffix" class="el-input__icon el-icon-search"></i>
          </el-input>
          <div
            @click="handleTitleClick(item, index)"
            v-for="(item,index) in titleMsg"
            :key="index"
            class="text"
            :class="{ 'clicked': item.click }"
          >
            <span>{{item.labelName + `（`+item.docCount + `）` }}</span>
          </div>
          <el-pagination
            small
      ...
      <el-container>
        <!-- <el-header>
          <el-input placeholder="请输入您要查找的关键字..." v-model="searchMsg">
            <template slot="suffix">
              <el-button @click="handleKeySearch" type="primary">开始搜索</el-button>
            </template>
          </el-input>
        </el-header> -->
        <el-main>
          <!-- <el-card  class="right-card" style="height:100%;padding:0px 20px 10px 20px;overflow-y: auto;" v-if="isLabelSearch">
            ...
            <div class="topsearch">
              ...
              <div class="searchdoc">
                <el-input v-model="searchMsg" placeholder="请输入..." clearable  size="small" @keyup.enter.native="handleKeySearch()">
                <i slot="suffix" class="el-input__icon el-icon-search" @click="handleKeySearch"></i>
                </el-input>
              </div>
            </div>
  ...
  methods: {
    ...
    handleSpaceClick(item, index) {
      ...
      this.$nextTick(() => {
        ...
        this.$set(this.spaceMsg[index], "click", true)
      })
      this.handleKeySearch();
      console.log(1111, this.spaceMsg)
    },
    // 获取空间信息分页列表
    getSpaceInfos() {
