    ...
    <el-card class="mess-card" shadow="hover">
      ...
      <div class="body">
          <el-form :model="ruleForm" ref="ruleForm" label-width="80px">
              <el-form-item :label="$route.params.type === 'check'?'发起人：':'受理人：'" prop="people">
              ...
              :rules="[{ required: true, message: '请输入受理信息', trigger: 'blur' }]">
                 <el-input type="textarea" v-model="ruleForm.mess" :autosize="{ minRows: 2, maxRows: 5}" class="textInput" disabled></el-input>
              </el-form-item>
          </el-form>
      </div>
      <div class="footer">
          <div style="text-align:center;width:100%" v-if="$route.params.type === 'confirm'&&!$route.params.notMyTask">
              <el-button  type="primary" size="mini" @click="handleSubmitClose" >关 闭</el-button>
              <!-- <el-button size="mini" @click="handleSubmit(false)">驳 回</el-button> -->
          </div>
          <div style="text-align:center;width:100%" v-else-if="$route.params.type === 'back'">
              <el-button  type="primary" size="mini" @click="handleSubmitAgain" v-if="$route.params.notMyTask===undefined">继续办理</el-button>
              <el-button size="mini" @click="handleSubmitClose" v-if="!$route.params.notMyTask">关闭</el-button>
          </div>
      </div>
    </el-card>
  </div>
...
export default {
  ...
  methods: {
    ...
    getTableData() {
      ...
      if (this.$route.params.type === 'confirm' || this.$route.params.Status === '已关闭') {
        params.onlyUnpub = false;
      }
      ...
      this.$confirm('确定要关闭该工单吗?', '提示', {
        ...
        if (this.saveLocking) {
          ...
          if (this.$route.params.type === 'back') {
            ...
            get('/home/matter/stdrelease/closeCase', {
              caseId: this.caseId,
              taskId: this.taskId
            }).then(res => {
              this.saveLocking = true;
    ...
    handleSubmitAgain() {
      this.closeSelectedTag2(this.$route);
  ...
  created() {
    ...
    if (this.$route.params.type === 'confirm') {
      this.getCloseDetail('/home/matter/stdrelease/getCloseCasePageInfo');
    } else if (this.$route.params.type === 'back') {
      this.getCloseDetail("/home/matter/stdrelease/getModCasePageInfo");
    } else if (this.$route.params.type === 'check') {
      this.getCloseDetail("/home/matter/stdrelease/getApprocePageInfo");
    } else if (this.$route.params.Status === '已关闭') {
      this.getCloseDetail("/home/matter/stdrelease/getCloseCasePageInfo");
    }
    // this.getTableData();
  }
